...
startSeednode() {
  ...
  if [ -n "$NODE_NICENESS" ]
  then
    seednodeCommand="nice -n $NODE_NICENESS ./seednode -log-save -log-level $LOGLEVEL |& tee stdout.txt"
  else
    seednodeCommand="./seednode -log-save -log-level $LOGLEVEL |& tee stdout.txt"
  fi
